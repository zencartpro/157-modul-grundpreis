 ...
 * @version $Id: product_listing.php for Grundpreis 2022-04-11 18:52:16Z webchills $
 */
  ...
  while (!$listing->EOF) {
    ...
    for ($col=0, $n=count($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        if (zen_not_null($listing->fields['products_base_unit_price']) && $listing->fields['products_base_unit_price']<>0 ) {
	$lc_price .= '<br>'.TEXT_PRODUCTS_BASE_UNIT_PRICE . ' ' . $currencies->format(zen_round($listing->fields['products_base_unit_price'],3)).'/'.$listing->fields['products_base_unit'].'<br>';
        }
	}
        $lc_align = 'right';
        if ($product_listing_layout_style == 'columns') $lc_align = 'center';
