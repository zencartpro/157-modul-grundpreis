 ...
 * @version $Id: new_products.php for Grundpreis 2023-11-14 17:10:16Z webchills $
 */
...
if ((($manufacturers_id > 0 && empty($_GET['filter_id'])) || !empty($_GET['music_genre_id']) || !empty($_GET['record_company_id'])) || empty($new_products_category_id)) {
    $new_products_query =
        "SELECT DISTINCT p.products_id, p.products_image, pd.products_name, p.products_price, p.products_base_unit_price, p.products_base_unit, p.master_categories_id
...
if ($num_products_count > 0) {
    ...
    while (!$new_products->EOF) {
        ...
        if (!empty($new_products->fields['products_base_unit_price']) && $new_products->fields['products_base_unit_price']<>0 ) 
	      $products_price .= '<br><span class="basepricelist">'.TEXT_PRODUCTS_BASE_UNIT_PRICE . ' ' . $currencies->format(zen_round($new_products->fields['products_base_unit_price'],3)).'/'.$new_products->fields['products_base_unit'].'</span><br>';
        // end add base price
